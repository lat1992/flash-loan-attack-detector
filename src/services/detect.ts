...
export class DetectService {
  ...
  async detectExploit(blockNumber: number): Promise<ExploitInfo> {
    let isExploited = false;
    const attacks: Attack[] = [];
    await this.provider.getBlock(blockNumber, true).then(async (block) => {
      if (!block) {
        ...
        await this.provider
          ...
          .then(async (receipt) => {
            ...
            let [attacked, hasFlashLoan, targetToken, victim, exploitAmount] =
              this.analyzeEulerExploitPattern(receipt);
            if (attacked) {
              isExploited = true;
              ...
              let price: number = await priceService.getPrice(
                this.provider,
                targetToken,
                blockNumber,
              );
  ...
  private analyzeEulerExploitPattern(
  ...
  private findHighestKeys(map: Map<string, number | bigint>): string {
